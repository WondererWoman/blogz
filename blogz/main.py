...
class BlogHandler(webapp2.RequestHandler):
    ...
    def get_posts_by_user(self, limit, offset):
        """
        ...
        query = Post.all().filter('author=', 'author').order('-created')
        return query.fetch(username=username, limit=limit, offset=offset)
...
class LoginHandler(BlogHandler):
    ...
    def post(self):
        ...
        if not user:
            self.render_login_form(username=submitted_username, error="Invalid username")
        elif hashutils.valid_pw(submitted_username, submitted_password, user.pw_hash):
            self.login_user(user)
            self.redirect('/blog/newpost')
        else:
            self.render_login_form(username=submitted_username, error="Invalid password")
